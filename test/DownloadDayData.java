...
//		}
		...
		for (Contract contract : contracts) {
			...
			Date[] d=DayLong.getMinMaxAvailableHist(contract,Util.BAR_1_DAY);
			if (d[0]==null) {
				for (int i=0;i<15;i++) {
					SimpleDateFormat form = new SimpleDateFormat("yyyyMMdd HH:mm:ss");
					String formatted = form.format(cal.getTime());
					client.reqHistoricalData(request_id, contract, formatted, "1 Y", "1 day", "TRADES", 1, 1, null);
				    Thread.sleep(2000);
				...
				cal.add(Calendar.YEAR, -1);
				Date twelve_years_ago=cal.getTime();
				cal.setTime(min_date);
				while (twelve_years_ago.before(cal.getTime())) {
					SimpleDateFormat form = new SimpleDateFormat("yyyyMMdd HH:mm:ss");
					String formatted = form.format(cal.getTime());
					client.reqHistoricalData(request_id, contract, formatted, "1 Y", "1 day", "MIDPOINT", 1, 1, null);
				    Thread.sleep(2000);
				    cal.add(Calendar.YEAR, -1);
				}
			}
